...
function parseList(t){return[...new Set(t.split(/[\s,]+/).map(t=>t.trim()).filter(t=>t))]}
...
leaveBtn.onclick=async()=>{shouldStopSpam=!0;appendLog("Ex scen...");const t=parseList(tokensInput.value),e=guildInput.value.trim();if(!t.length||!e)return appendLog("Warning: 入力不足");for(const n of t)await leaveGuild(n,e);appendLog("Success: 退出完了");submitBtn.disabled=!1;submitBtn.classList.remove("loading");submitBtn.textContent="RAID START"};
